...
create('Imperial', serverOption)
        .then(client => {
            ...
            // Force untuk keep session saat ini
...
async function msgHandler (client, message) {
    try {
        ...
        if (cmd) {
                    ...
                    if (isMedia) {
                        ...
                        if (url.match(isUrl)) {
                            await client.sendStickerfromUrl(from, url, { method: 'get' })
                                .catch(err => console.log('Pengecualian: ', err))
                        } else {
                            client.sendText(from, 'Link image tidak valid')
                        }
                    } else {
                        client.sendText(from, 'Kirim image dengan caption #stiker untuk membuat sticker')
                    }
                    break
                case '#halo':
            ...
            if (!isGroupMsg) console.log('[RECV]', color(time, 'yellow'), 'Pesan dari', color(pushname))
            if (isGroupMsg) console.log('[RECV]', color(time, 'yellow'), 'Pesan dari', color(pushname), 'in', color(name))
        }
    } catch (err) {
...
process.on('Ada yang salah', function (err) {
    console.log('Pengecualian: ', err);
  });
...
function color (text, color) {
  switch (color) {
    ...
    default: return '\x1b[32m' + text + '\x1b[0m' // default hijau
  }
}
